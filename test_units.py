...
def test_validate_csv_filename_01():
    # Test filename not end with .csv
    with pytest.raises(
...
def test_add_filename_column_01():
    # Test add 'filename' column to dataframe
...
def test_read_csv_to_dataframe_01():
    # Test dataframe output of one .csv
    df1 = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
...
def test_read_csv_to_dataframe_02():
    # Test FileNotFoundError when given a file doesn't exist
    with pytest.raises(FileNotFoundError):
        csv_combiner._read_csv_to_dataframe("sdfe.csv")


def test_read_csv_to_dataframe_03():
    # Test ParserError when given erronous format data
...
def test_combine_csv_01(capsys):
    # Test final output of multiple .csv combined
    df1 = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
